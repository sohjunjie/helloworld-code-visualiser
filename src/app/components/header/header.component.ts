...
@Component({
  ...
  template: `
    <header class="h-16 px-6 glass-panel flex items-center justify-between border-b border-slate-700/50 sticky top-0 z-40 shadow-xl">
      ...
      >
        <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-sky-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-sky-500/20 group-hover:scale-105 group-active:scale-95 transition-transform duration-200 flex-shrink-0">
          <svg class="w-5 h-5 text-white flex-shrink-0" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      @if (store.analysisResult()) {
        <nav class="flex items-center space-x-1 bg-slate-900/80 p-1 rounded-xl border border-slate-800 shadow-inner">
          ...
          >
            <svg class="w-4 h-4 text-sky-400 flex-shrink-0" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="flex items-center space-x-3">
        ...
        <div class="relative group">
          <button class="btn-interactive btn-secondary px-3.5 py-1.5 text-xs font-semibold rounded-xl flex items-center space-x-1.5">
            ...
            <svg class="w-3.5 h-3.5 text-slate-400 group-hover:rotate-180 transition-transform duration-300 flex-shrink-0" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if (store.analysisResult()) {
          ...
          >
            <svg class="w-4 h-4 flex-shrink-0" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
